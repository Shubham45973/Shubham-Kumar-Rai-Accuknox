...
def check_eater_id_ordered_twice(logs):
    eater_ids = {}
    for log in logs:
...
def get_top_three_menu_items(logs):
    menu_items = {}
    if check_eater_id_ordered_twice(logs)[0]:
        raise Exception("Eater ID: {} has ordered twice".format(check_eater_id_ordered_twice(logs)[1]))
    for log in logs:
